...
def get_random_price_exercise() -> Tuple[str, pd.Series]:
    ...
    return price, item, number_row

def get_price_answer(number_row: pd.Series) -> str:
    if pd.notna(number_row['kiek_kainuoja_compound']):
        price_phrase = f"{number_row['kiek_kainuoja']} {number_row['kiek_kainuoja_compound']} {number_row['euro_acc']}."
    else:
        price_phrase = f"{number_row['kiek_kainuoja']} {number_row['euro_acc']}."
...
def Page():
    ...
    def new_exercise():
        new_price, new_item, new_number_row = get_random_price_exercise()
        set_price(new_price)
        set_item(new_item)
    ...
    if price is None or number_row is None or item is None:
        new_exercise()

    exercise_text = f"Kiek kainuoja {item}? ({price})" if price is not None and item is not None else ""
    ...
    with solara.Column(align="center", gap="20px"):
        with solara.Row(justify="center", gap="20px"):
            ...
            with solara.Column(gap="20px", classes=["w-1/2"]):
                with solara.Card("Exercise: Kiek kainuoja?", margin=10):
